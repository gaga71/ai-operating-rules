...
For ordinary execution, before the first substantive response in a new chat, when file search or file access is available, actively locate and load the current accessible file whose name matches `AI_OPERATING_RULES_YYYY-MM-DD_<main-short-sha>.md` from the current chat, project, or Library. Search the available file surfaces rather than waiting for automatic retrieval. The intended Library state contains only the newest active deployment file. Treat that file as the ordinary-execution deployment copy derived from `gaga71/ai-operating-rules` `main` / `adapters/chatgpt.md`.
...
- Files uploaded to or created in ChatGPT may be stored in Library and reused across chats, but retrieval can fail in some surfaces or workspace configurations. The loader therefore requires active lookup when file search or file access is available; if the current deployment file still cannot be accessed, explicitly attach or select it when reliable application matters.
